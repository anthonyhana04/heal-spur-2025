 ...
 */
export async function createRoom(env: Env, userId: string, initialTitle?: string): Promise<RoomMeta> {
  const now = Date.now();
  const id = uuidv7();
  const meta: RoomMeta = {
    id,
 ...
 */
export async function listRooms(env: Env, userId: string): Promise<RoomMeta[]> {
 ...
 */
export async function saveMessage(env: Env, userId: string, roomId: string, role: Role, content: string): Promise<ChatMessage> {
  const id  = uuidv7();
 ...
 */
export async function getMessages(env: Env, userId: string, roomId: string, cursor: string | null, limit = 32): Promise<ChatMessage[]> {
